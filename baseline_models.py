...
class ScorePredictor(nn.Module):
    ...
    def ultragcn_forward(self, edge_subgraph, x):
        with edge_subgraph.local_scope():
            edge_subgraph.ndata['x'] = x
            # edge_subgraph.data['b'] = edge_subgraph.indgrees()
            for etype in edge_subgraph.canonical_etypes:
                if etype == ('item', 'iu', 'user'):
                    continue
                edge_subgraph.nodes[etype[0]].data['b'] = edge_subgraph.out_degrees(etype = etype)
                edge_subgraph.nodes[etype[2]].data['b'] = edge_subgraph.in_degrees(etype = etype)
                edge_subgraph.apply_edges(
                    dgl.function.u_dot_v('x', 'x', 'score'), etype=etype)
                edge_subgraph.apply_edges(lambda edges:{'beta':(edges.dst['b']+1/edges.src['b']+1)**(1/2)/edges.dst['b']}, etype=etype)
            return edge_subgraph.edata['beta'],edge_subgraph.edata['score']
                ...
                if pretrained_feature != None:
                    ...
                    if att != 0:
                        if att == 1:
                            ...
                            # print(graph.nodes[dst].data['q'].shape) # size = (n,1,64)
                            # print(graph.nodes[dst].data['k'].shape) # size = (n,64,2)
...
class LightGCN(nn.Module):
    def __init__(self, args, graph, device):
        ...
        self.combine_view = args.combine_view
        self.combine_alpha = args.combine_alpha
    ...
    def dhcf_forward(self, graph):
        ...
        H_user_sparse = graph.adj(etype='ui')
        H_user2 = H_user_sparse @ H_user_sparse.T @ H_user_sparse
        H_edges_user2 = H_user2.coo()
        ...
        H_item_sparse = graph.adj(etype='iu')
        H_item2 = H_item_sparse @ H_item_sparse.T @ H_item_sparse
        H_edges_item2 = H_item2.coo()
    ...
    def forward(self, graph, pre_train=True, trans_conv=True):
        ...
        if pre_train and trans_conv and self.att_conv!=-1:
    ...
    def create_ssl_loss_user(self,ini_h_user_v1,ini_h_user_v2, ssl_temp = 0.1, ssl_reg = 1e-4, k=4, reg = False):
        # ssl_temp = 0.1
        ...
        for i in range(k-1,k):
            ...
            h_user_v1 = torch.nn.functional.normalize(h_user_v1, p=2, dim=1)
            h_user_v2 = torch.nn.functional.normalize(h_user_v2, p=2, dim=1)
        ...
        if reg:
            regularizer = reg * (1 / 2) * (self.user_embedding.norm(2).pow(2) +
                                     self.item_embedding.norm(2).pow(2))
    ...
    def create_bpr_loss(self, pos_g, neg_g, h, users_non_induct=None, loss_type='bpr'):
        if loss_type == 'align':
            ...
            mf_loss = mf_loss.mean()
        elif loss_type == 'ultragcn':
            sub_fig_feature = {'user': h['user'], 'item': h['item']}
            pos_beta, pos_score = self.pred.ultragcn_forward(pos_g, sub_fig_feature)
            # print(pos_score[('user', 'ui', 'item')].squeeze().shape)
            # print(pos_beta[('user', 'ui', 'item')].shape)
            pos_score = torch.mul(pos_score[('user', 'ui', 'item')],pos_beta[('user', 'ui', 'item')].unsqueeze(dim=1)) + pos_score[('user', 'ui', 'item')]
            neg_beta, neg_score = self.pred.ultragcn_forward(neg_g, sub_fig_feature)
            neg_score = torch.mul(neg_score[('user', 'ui', 'item')],neg_beta[('user', 'ui', 'item')].unsqueeze(dim=1)) + neg_score[('user', 'ui', 'item')]
            pos_score = pos_score.repeat_interleave(self.neg_samples, dim=0)
            mf_loss = neg_score - pos_score
            mf_loss = mf_loss.mean()
